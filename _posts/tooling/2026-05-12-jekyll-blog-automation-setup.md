...
플러그인 자유도가 꼭 필요 없다면 마지막이 가장 편했습니다. 레포 이름을 `<username>.github.io` 로 두면 GH Pages 가 user site 로 자동 인식하고, main 브랜치 루트를 곧바로 사이트 소스로 본다는 점이 결정타였습니다.
...
여기서부터가 본론입니다. 환경이 1분만 거추장스러워도 글쓰기는 이어지기 어렵습니다. 새 포스트마다 파일명·frontmatter·날짜를 직접 채우는 작업조차 충분히 큰 마찰입니다.
...
GitHub Actions 로 직접 빌드하는 구성은 자유도가 높지만, 워크플로 YAML 자체가 또 하나의 유지보수 대상이 됩니다. 화이트리스트 외 플러그인이 필요 없으면 GH Pages 의 내장 빌더가 더 단순합니다. "자유도가 필요해질 때 마이그레이션" 을 default 로 두고 시작하는 게 낫습니다.
