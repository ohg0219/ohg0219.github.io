...
배포 단계에서 가장 자주 듣는 이 한 문장이, 컨테이너라는 발상이 태어난 출발점입니다. 같은 코드가 개발자 PC, 동료 PC, 스테이징, 운영에서 각자 다르게 동작하는 문제를 줄이려는 시도. 그 결과물이 Docker 입니다.

이 글은 Docker 를 처음 만질 때 반드시 잡아야 할 **다섯 가지 기본** 을 정리합니다. Docker 가 무엇인지, 가상머신과 어떻게 다른지, 이미지와 컨테이너의 관계, 첫 컨테이너를 띄우는 방법, 그리고 현장에서 자주 쓰는 명령어들입니다.

시리즈의 1편이고, 다음 편부터는 네트워크·Compose·볼륨·이미지 빌드 심화로 한 단계씩 이어갑니다.
...
가장 자주 헷갈리는 부분입니다. 둘 다 "격리된 실행 환경" 을 만든다는 점은 비슷하지만, 격리의 단위가 다릅니다. 그림으로 보면 차이가 분명해집니다.
...
처음부터 모든 명령어를 외울 필요는 없습니다. 컨테이너를 띄우고, 보고, 들어가고, 지우는 데 필요한 것만 추리면 다음 정도입니다.
...
Docker 의 모든 명령어 (`run`, `ps`, `images`, `logs`, `exec`) 는 결국 이미지와 컨테이너 중 어느 쪽을 다루는지로 나뉩니다. 명령어 앞에서 *"이건 이미지를 만지는 건가, 컨테이너를 만지는 건가?"* 한 번만 짚으면 옵션도 훨씬 덜 헷갈립니다.
